...
# this file came from https://github.com/GaoFangshu/ELF-example
#  but it doesn't work, so forget it
...
from rlpytorch import (ArgsProvider, ModelInterface, ModelLoader, Sampler, Trainer)
...
def remote_init(mi, gpu_id, args):
    ...
    if gpu_id != 0:
        ...
        return dict(
            rl_method=rl_method,
            rl_method_local=rl_method_local,
            mi=mi,
            mi_local=mi_local,
            count=0,
            gpu_id=gpu_id
...
if __name__ == '__main__':
    ...
    if use_multi_process:
        ...
        if eval_only:
            ...
            GC.reg_callback("actor", trainer.actor)
            runner.setup(
                GC, episode_summary=trainer.episode_summary, episode_start=trainer.episode_start
            )
