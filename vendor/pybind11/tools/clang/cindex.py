...
class TranslationUnit(ClangObject):
    ...
    def from_source(cls, filename, args=None, unsaved_files=None, options=0, index=None):
        ...
        if args_length > 0:
            args = (arg.encode('utf8') if isinstance(arg, str) else arg for arg in args)
        ...
        ptr = conf.lib.clang_parseTranslationUnit(
            index, filename, args_array, args_length, unsaved_array, len(unsaved_files), options
        )
...
functionList = [
    ("clang_annotateTokens", [TranslationUnit,
                              POINTER(Token), c_uint,
                              POINTER(Cursor)]),
    ("clang_CompilationDatabase_dispose", [c_object_p]),
    (
        "clang_CompilationDatabase_fromDirectory", [c_char_p, POINTER(c_uint)], c_object_p,
    ...
    (
        "clang_CompilationDatabase_getCompileCommands", [c_object_p, c_char_p], c_object_p,
        CompileCommands.from_result
    ),
    ...
    (
        "clang_codeCompleteAt", [TranslationUnit, c_char_p, c_int, c_int, c_void_p, c_int, c_int],
    ...
    (
        "clang_parseTranslationUnit", [Index, c_char_p, c_void_p, c_int, c_void_p, c_int, c_int],
        c_object_p
    ),
    ...
    ("clang_tokenize", [TranslationUnit, SourceRange,
